...
from asyncio import Lock, TimerHandle, Task
import time
import uuid
...
@register("context_enhancer_v2", "木有知", "智能群聊上下文增强插件 v2.3", "2.3.2", repo="https://github.com/muyouzhi6/astrbot_plugin_context_enhancer")
class ContextEnhancerV2(Star):
    ...
    async def terminate(self, context: Context):
        ...
        logger.info("[ContextEnhancerV2] 正在执行最终的上下文缓存保存...")
        if await self._save_cache_to_file(is_final_save=True):
            logger.info(f"上下文缓存已成功保存到 {self.cache_path}")
        else:
            logger.error("[ContextEnhancerV2] 最终的上下文缓存保存失败。")
    ...
    def _load_plugin_config(self) -> PluginConfig:
        return PluginConfig(
            ...
            # --- 默认保存间隔已设置为 220 秒 ---
            autosave_interval_seconds=self.raw_config.get("autosave_interval_seconds", 220),
    ...
    async def _save_cache_to_file(self, is_final_save: bool = False):
        ...
        try:
            ...
            if not is_final_save:
                logger.debug(f"[ContextEnhancerV2] 上下文缓存已自动保存到 {self.cache_path}")
            return True
    ...
    async def _periodic_save_task(self):
        if self.config.autosave_interval_seconds <= 0:
            logger.info("[ContextEnhancerV2] 定时保存功能已禁用 (autosave_interval_seconds <= 0)。")
            return

        logger.info(f"[ContextEnhancerV2] 定时保存任务已启动，保存间隔: {self.config.autosave_interval_seconds} 秒。")
        try:
            while True:
                await asyncio.sleep(self.config.autosave_interval_seconds)
                await self._save_cache_to_file()
        except asyncio.CancelledError:
            logger.info("[ContextEnhancerV2] 定时保存任务被取消，正在关闭。")
        except Exception as e:
            logger.error(f"[ContextEnhancerV2] 定时保存任务发生致命错误: {e}", exc_info=True)
